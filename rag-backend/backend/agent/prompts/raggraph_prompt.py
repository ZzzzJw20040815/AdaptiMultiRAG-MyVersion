...
**⚠️ 引用格式要求（严格遵守）：**
- 使用方括号数字格式引用来源，如：这是一个重要发现 [1]。另一项研究也证实了这点 [2]。
- 多个来源可以合并：两者都采用这种方法 [1][2]。
- 在正文末尾的「参考文献」部分，按数字顺序列出对应的文档名称
- **严格要求**：如果某个陈述只有一个来源支持，不要使用"都"、"均"、"两者皆"等暗示多来源的词汇
- 只有当多个来源都明确支持同一观点时，才能说"都"，并且必须同时引用多个来源如 [1][2]

**⚠️ 知识库文献限制（必须遵守）：**
知识库中只有以下 {available_doc_count} 篇文献，你**只能引用**这些文献，**绝对不能**引用任何不在此列表中的文献：
{available_docs}

即使检索内容中提到了其他论文名称、系统名称或方法名称，也**不能**将它们作为独立的参考文献引用。
...
该系统通过语言反馈实现机器人行为的实时修正 [1]。两篇论文都提出了分层控制架构 [1][2]，其中高层策略以自然语言为输出媒介。
